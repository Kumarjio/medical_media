             ...
             <td>
              <?php $cs_master = $this->db->query("select * from pro_type")->result_array(); ?>
                                             ...
                                             <?php foreach($cs_master as $val) { ?>
                                               ...
                                               <select class="selectpicker form-control "     name="pname1" id="item_name1_1"   data-live-search="true" data-live-search-placeholder="Search" data-actions-box="true">
					...
					{ 
                        ...
                        <option value="<?php echo $row->i_id; ?>"><?php echo $row->i_name; ?>-<?php echo $row->ime_serial; ?></option>
							<?php 
                                               ...
                                               <?php /*?><td>
                                          <span class="probs1">       <select  class="selectpicker form-control uppercase itemsq2"  name="item_name1[]" id="item_name1_1"  data-live-search="true" data-live-search-placeholder="Search" data-actions-box="true">
										  <option value="" selected disabled>Select</option>
										  <?php foreach($item as $row){ if($row->i_category == 1) {?>
                                          
                                          <option value="<?php echo $row->i_id; ?>"><?php echo $row->i_name ?></option><?php }} ?></select>
                                          </span>
												<span class="probes1" style="display:none">	<select style="display:none" class="selectpicker form-control uppercase itemsq3"  name="item_name2[]" id="item_name2_1"  data-live-search="true" data-live-search-placeholder="Search" data-actions-box="true"><option value="" selected disabled>Select</option><?php foreach($item as $row){ if($row->i_category == 2) {?><option value="<?php echo $row->i_id; ?>"><?php echo $row->i_name ?></option><?php }} ?></select></span>
												</td><?php */?>
                                                 <td>
                    <input type="number"  class="form-control uppercase" readonly name="minqty" id="minqty_1" /></td>
                    <?php /*?><td>
...
$(document).ready(function(){
	...
	function fnadd() 
		...
		+'<td class="no_disp'+a+'" id="disp_id'+a+'<?=$val['id']?>"<?=($temp_id > '0')?'style="display:none"':''?>><select class="selectpicker form-control " name="pname1" id="item_name1_'+a+'" data-live-search="true" data-live-search-placeholder="Search" data-actions-box="true"><option disabled selected value="">Select Product</option><?php foreach($item as $row){if($val['id'] == $row->i_category){?><option value="<?php echo $row->i_id; ?>"><?php echo $row->i_name; ?>-<?php echo $row->ime_serial; ?></option><?php }}?></select><?php $temp_id++; } ?></td>'
		+'<td><input type="number"  class="form-control uppercase" readonly name="minqty" id="minqty_'+a+'" /></td>'
...
$(document).on('change','.itemsq4',function()
{
		var cus = $(this).val();
		var id= $(this).attr('id');
		sid = id.split('_');
...
$(document).on('change','.itemsq3',function()
{
		var cus = $(this).val();
		var id= $(this).attr('id');
		sid = id.split('_');
